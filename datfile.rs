...
impl DatFile {
    ...
    pub fn find_file(&mut self, search_id: u32) -> IoResult<(i64, uint)> {
        ...
        loop {
            ...
            while i < num_files {
                if search_id <= words[NUM_FILES_OFF + 1 + i * 6 + 1] {
                    break;
                }
                i += 1;
            }

            if i < num_files && search_id == words[NUM_FILES_OFF + 1 + i * 6 + 1] {
                let loc = words[NUM_FILES_OFF + 1 + i * 6 + 2] as i64;
                let size = words[NUM_FILES_OFF + 1 + i * 6 + 3] as uint;
                return Ok((loc, size));
            }
